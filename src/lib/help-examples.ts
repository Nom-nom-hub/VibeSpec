...
export function getExampleHelp(commandName: string): string {
  const examples = helpExamples[commandName as keyof typeof helpExamples];
  if (!examples) return '';
...
export function getCommandQuickTip(commandName: string): string {
