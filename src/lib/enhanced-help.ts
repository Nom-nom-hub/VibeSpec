 ...
 */
export function getEnhancedHelp(commandName: string): string {
  const help = helpExamples[commandName as keyof typeof helpExamples];
  if (!help) return '';
 ...
 */
export function getCommandReference(): string {
  let output = '';
 ...
 */
export function addExamplesToCommander(cmd: any, commandName: string): void {
  const help = helpExamples[commandName as keyof typeof helpExamples];
